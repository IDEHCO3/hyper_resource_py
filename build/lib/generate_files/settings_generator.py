...
SETTING_TEMPLATE_PATH = PACKAGE_PATH + '/settings_template.py'
USER_CURRENT_PATH = os.getcwd().replace('\\', '/')
#USER_CURRENT_IMPORT_PATH = USER_CURRENT_PATH.replace("/", ".")
...
def generate_file(package_name, app_name, generate_db='sqlite'):
    original_settings_path = USER_CURRENT_PATH+ "/" + package_name + '/settings.py'
    #original_settings_path_dot_notation = package_name + '.settings'

    if os.path.isfile(package_name + '/original_settings.py'):
        original_settings_path = USER_CURRENT_PATH + "/" + package_name + '/original_settings.py'
        #original_settings_path_dot_notation = package_name + ".original_settings"

    with open(original_settings_path, 'r') as original_settings_file:
        with open(SETTING_TEMPLATE_PATH, 'r') as template_setting_file:

            sys.path.append(USER_CURRENT_PATH)
            original_settings_module = importlib.import_module(package_name + ".settings")
    ...
    ORIGINAL_SETTINGS_PATH = USER_CURRENT_PATH + "/" + package_name + "/original_settings.py"
    NEW_SETTINGS_PATH = USER_CURRENT_PATH + "/" + package_name + '/new_settings.py'
    DEFAULT_SETTING_PATH = USER_CURRENT_PATH + "/" + package_name + '/settings.py'
    with open(NEW_SETTINGS_PATH, 'w') as new_settings:
        new_settings.write(new_settings_data)

    if not os.path.isfile(ORIGINAL_SETTINGS_PATH):
        os.rename(DEFAULT_SETTING_PATH, ORIGINAL_SETTINGS_PATH)

    if os.path.isfile(DEFAULT_SETTING_PATH):
        os.remove(DEFAULT_SETTING_PATH)
    os.rename(NEW_SETTINGS_PATH, DEFAULT_SETTING_PATH)
